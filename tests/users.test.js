...
describe('2: Testing userId errors', () => {
  ...
  const doesntExistErrorText = `Record with userId '${nonExistingId}' doesn't exist`;
  const invalidUserIdErrorText = `Invalid userId: '${invalidId}'`;
  ...
  it('With a GET api/user/{userId} request, we try to get non existing record', async () => {
    ...
    expect(response.status).toBe(404);
    expect(response.text).toBe(doesntExistErrorText);
  });
  ...
  it('With a GET api/user/{userId} request, we try to get record using invalid id', async () => {
    ...
    expect(response.text).toBe(invalidUserIdErrorText);
  });
...
describe('3: Testing data errors', () => {
  let id = '';
  const request = supertest(server);
  const errorText = `Request should contain required fields in json format: 'username, age, hobbies'`;

  const notFullyFilledUserData = {
    username: 'Valera',
  }
  
  const incorrectUserData1 = {
    username: 32,
    age: 'qwe',
    hobbies: ['programming'],
  }

  const incorrectUserData2 = {
    username: 'Valera',
    age: 32,
    hobbies: 'programming',
  }
  ...
  it('With a PUT api/users/{userId} request, we try to update record using invalid Content-type', async () => {
    const response = await request
      .put('/api/users/a97fac64-0ccb-4cba-9014-37b65ed28ea7')
      .set('Content-type', 'text/plain')
      .send(JSON.stringify(userData));
    expect(response.status).toBe(400);
    expect(response.text).toBe(errorText);
  ...
  it('With a PUT api/users/{userId} request, we try to update record with incorrect data fields', async () => {
    const response = await request
      .put(`/api/users/${id}`)
      .set('Content-type', 'application/json')
      .send(JSON.stringify(incorrectUserData1));
    expect(response.status).toBe(400);
    expect(response.text).toBe(errorText);
  });
  ...
  it('With a PUT api/users/{userId} request, we try to update record with incorrect data fields', async () => {
    const response = await request
      .put(`/api/users/${id}`)
      .set('Content-type', 'application/json')
      .send(JSON.stringify(incorrectUserData2));
    expect(response.status).toBe(400);
    expect(response.text).toBe(errorText);
  });
});
